...
$(document.body).on("click", ".refSave", function(){
    let input = $(this).closest(".form-newPanel").find('.input100');
    if(checkInputs(input)){
        $.post("requisicoesRefeicoes.php", {"nome":input.val(), "criar":true}, function(data, status) {
            if (status=='success' && /\d+/.test(data)) {
                let div = `<div class="col-sm-12 refeicaoMostrada">
                                <div class="panel panel-default">
                                    ...
                                    <a href="refeicaoSolo.php?idRef=${data}&nome=${input.val()}">
                                        <button type="button" class="login100-form-btn panel-footer">Visualizar</button>
                                    </a>
                                </div>
                            </div>`;
...
$(document.body).on("click", ".refDelete", function(){
    if(confirm("Confirmar deleção?")){
        ...
        $.post("requisicoesRefeicoes.php", {"idRefeicao":idRef, "deletar":true}, function(data, status) {
            if (status=='success' && data == 1) {
...
$(".addAlimento").click(function(){
    ...
    if (checkInputs(input)) {
        let dados = $(".formAddAlimento").serialize()+"&idRefeicao="+$(".escolheAlimento").data("id")+"&editar="+true;
        $.post("requisicoesRefeicoes.php", dados, function(data, status){
...
$(document.body).on("click", ".alimentoDelete", function(){
    if(confirm("Confirmar deleção?")){
        ...
        $.post("requisicoesRefeicoes.php", {"idRefeicao":idRef, "idAlimento":idAlim, "editar":true}, function(data, status) {
            if (status=='success' && data == 1) {
